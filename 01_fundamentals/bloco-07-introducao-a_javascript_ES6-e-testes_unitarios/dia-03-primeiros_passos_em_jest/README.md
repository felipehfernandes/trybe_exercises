...
- Verifique se a chamada `myRemove([1, 2, 3, 4], 3)` _não_ retorna o array [1, 2, 3, 4]
...
- Para a função `encode` teste se as vogais _a, e, i, o, u_ são convertidas em 1, 2, 3, 4 e 5, respectivamente;
- Para a função `decode` teste se os números 1, 2, 3, 4 e 5 são convertidos nas vogais _a, e, i, o, u_, respectivamente;
...
Implemente a função `techList` a partir dos testes abaixo. Experimente refatorar a função que você criou para esse projeto! _É importante nunca alterar os testes ou as variáveis já escritas no código_.
...
Implemente a função `hydrate` a partir dos testes abaixo. Experimente refatorar a função que você criou para o projeto Playground Function! _É importante nunca alterar os testes ou as variáveis já escritas no código._
